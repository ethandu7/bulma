...
EMPTY_OBJECT(Map, g_macros, macros);
EMPTY_OBJECT(Map, g_once, once);
EMPTY_OBJECT(Map, g_keywords, keywords);
EMPTY_OBJECT(Map, g_include_guard, include_guard);
...
static CondIncl *make_cond_incl(bool wastrue) {
    CondIncl *r = (CondIncl *)calloc(1, sizeof(CondIncl));
    r->ctx = IN_THEN;
    r->wastrue = wastrue;
    return r;
}
...
static Macro *make_macro(Macro *tmpl) {
    Macro *r = (Macro *)malloc(sizeof(Macro));
    *r = *tmpl;
    return r;
}
...
static void propagate_space(Vector *tokens, Token *tmpl) {
    if (vec_len(tokens) == 0)
        return;
    Token *tok = copy_token((Token *)vec_head(tokens));
...
static Vector *add_hide_set(Vector *tokens, Set *hideset) {
    ...
    for (int i = 0; i < vec_len(tokens); i++) {
        Token *t = copy_token((Token *)vec_get(tokens, i));
...
static void glue_push(Vector *tokens, Token *tok) {
    Token *last = (Token *)vec_pop(tokens);
...
static Token *stringize(Token *tmpl, Vector *args) {
    ...
    for (int i = 0; i < vec_len(args); i++) {
        Token *tok = (Token *)vec_get(args, i);
...
static Vector *subst(Macro *macro, Vector *args, Set *hideset) {
    ...
    for (int i = 0; i < len; i++) {
        Token *t0 = (Token *)vec_get(macro->body, i);
        Token *t1 = (i == len - 1) ? NULL : (Token *)vec_get(macro->body, i + 1);
        bool t0_param = (t0->kind == TMACRO_PARAM);
        bool t1_param = (t1 && t1->kind == TMACRO_PARAM);

        if (is_keyword(t0, '#') && t1_param) {
            vec_push(r, stringize(t0, (Vector *)vec_get(args, t1->position)));
            i++;
            continue;
        }
        if (is_keyword(t0, KHASHHASH) && t1_param) {
            Vector *arg = (Vector *)vec_get(args, t1->position);
            ...
            if (t1->is_vararg && vec_len(r) > 0 && is_keyword((Token *)vec_tail(r), ',')) {
                if (vec_len(arg) > 0)
                    vec_append(r, arg);
                else
                    vec_pop(r);
            } else if (vec_len(arg) > 0) {
                glue_push(r, (Token *)vec_head(arg));
        ...
        if (t0_param && t1 && is_keyword(t1, KHASHHASH)) {
            hideset = t1->hideset;
            Vector *arg = (Vector *)vec_get(args, t0->position);
            if (vec_len(arg) == 0)
                i++;
            else
                vec_append(r, arg);
...
static void unget_all(Vector *tokens) {
    for (int i = vec_len(tokens) - 1; i >= 0; i--)
        unget_token((Token *)vec_get(tokens, i));
}
...
static Token *read_expand_newline() {
    ...
    if (tok->kind != TIDENT)
        return tok;
    char *name = tok->sval;
    Macro *macro = (Macro *)map_get(macros, name);
...
static void hashhash_check(Vector *v) {
    if (vec_len(v) == 0)
        return;
    if (is_keyword((Token *)vec_head(v), KHASHHASH))
        errort((Token *)vec_head(v), "'##' cannot appear at start of macro expansion");
    if (is_keyword((Token *)vec_tail(v), KHASHHASH))
        errort((Token *)vec_tail(v), "'##' cannot appear at end of macro expansion");
}
...
static Vector *read_funclike_macro_body(Map *param) {
    ...
    for (;;) {
        ...
        if (tok->kind == TNEWLINE)
            return r;
        if (tok->kind == TIDENT) {
            Token *subst = (Token *)map_get(param, tok->sval);
            if (subst) {
...
static void read_ifndef() {
    ...
    if (tok->count == 2) {
        ...
        CondIncl *ci = (CondIncl *)vec_tail(cond_incl_stack);
...
static void read_endif(Token *hash) {
    ...
    CondIncl *ci = (CondIncl *)vec_pop(cond_incl_stack);
    expect_newline();
...
static char *join_paths(Vector *args) {
    Buffer *b = make_buffer();
    for (int i = 0; i < vec_len(args); i++)
        buf_printf(b, "%s", tok2s((Token *)vec_get(args, i)));
    return buf_body(b);
}
...
static bool guarded(char *path) {
    char *guard = (char *)map_get(include_guard, path);
...
static void read_include(Token *hash, File *file, bool isimport) {
    ...
    if (!std) {
        char cur[] = ".";
        char *dir = file->name ? dirname(strdup(file->name)) : cur;
        if (try_include(dir, filename, isimport))
            return;
    }
    for (int i = 0; i < vec_len(std_include_path); i++)
        if (try_include((char *)vec_get(std_include_path, i), filename, isimport))
            return;
  err:
...
static void read_include_next(Token *hash, File *file) {
    ...
    // workaround for the goto crosses the initialization of 'i' and 'cur'
    {
        char *cur = fullpath(file->name);
        int i = 0;
        for (; i < vec_len(std_include_path); i++) {
            char *dir = (char *)vec_get(std_include_path, i);
            if (!strcmp(cur, fullpath(format("%s/%s", dir, filename))))
                break;
        }
        for (i++; i < vec_len(std_include_path); i++)
            if (try_include((char *)vec_get(std_include_path, i), filename, false))
                return;
    }
  err:
    errort(hash, "cannot find header file: %s", filename);
...
void read_from_string(char *buf) {
    ...
    for (int i = 0; i < vec_len(toplevels); i++)
        emit_toplevel((Node *)vec_get(toplevels, i));
    stream_unstash();
}
